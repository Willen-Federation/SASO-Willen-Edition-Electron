...
import { Save, Eye, EyeOff, CheckCircle, ExternalLink, RefreshCw, Trash2, AlertTriangle, CloudOff, Activity, XCircle, CircleSlash } from 'lucide-react'
import { useFeatureFlags } from '../stores/useFeatureFlags'
import { useSyncQueue } from '../stores/useSyncQueue'
import type { AuthUser, PendingSyncOp, ReadinessStatus, ReadinessProbeStatus } from '@shared/types'
...
function probeBadge(status: ReadinessProbeStatus): { label: string; cls: string; Icon: typeof CheckCircle } {
  switch (status) {
    case 'ok':
...
function ReadinessSection({ autoRun }: { autoRun: boolean }) {
          ...
          <div className="space-y-1">
            {report.checks.map((check) => {
              ...
              return (
                <div
                  key={check.name}
                  className="flex items-start gap-2 text-xs border border-gray-100 rounded-md px-2 py-1.5"
                >
      ...
      <div className="card space-y-5">
        ...
        {tab === 'auth' && (
          <div className="space-y-4">
            ...
            <TestConnectionRow />
            <ReadinessSection autoRun={searchParams.get('diagnose') === '1'} />
